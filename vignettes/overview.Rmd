...
The purpose of a **two sample Z-test** is to test for the difference of means between two samples from two population distributions. The Z-test is a parametric test that requires the two samples to be normally distributed and performs well when you have large sample sizes and known population variances. To conduct this test on persistent landscapes, the test statistic is the integral of the mean persistent landscape. If these assumptions are not met, then the test is inappropriate. For more information, please reference section 3 of Bubenik (2015).

In our example, the null hypothesis is that there is not a difference in the two mean persistence landscapes, which would imply that they both came from the same population distribution. And the alternative hypothesis is that there is a difference between the mean persistence landscapes. What interval this integration is taken over is dependent on the `support` provided. Users can enter intervals for the `support` parameter to focus on regions where important topological features lie, this also would increase efficiency as there would be less calculations to be made. Then, the Z-score is computed from which we get our $p$-value. We can adjust $\alpha$ by changing the `conf.level`. Here, we will use the default $0.95$ confidence level, so $\alpha=0.05$.
