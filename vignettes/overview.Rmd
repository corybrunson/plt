...
We will now discuss how to perform hypothesis tests on persistence landscapes. In statistics, we can use hypothesis tests to weigh the evidence to determine whether two samples came from the same distribution. A hypothesis test requires a calculated test statistic from sample and an understanding of the distribution of the test statistic. Recall that the p-value is the probability of observing data as extreme as or more extreme than the observed data, assuming the null hypothesis is true. If the p-value is less than $\alpha$, then we reject our null hypothesis, otherwise we fail to reject it.
...
For our next example, we show how to make such a distinction. First, we create a counterfeit lemniscate sampler which may be similar enough to deceive quality control. The distinction in this counterfeit lemniscate is that it has different-sized circles, while the original lemniscate has same-sized lobes. Below, the non-symmetric lemniscate sampler is comprised of two uniform circles (of different radii), that are each sampled with noise and interlinked.
...
Now, we noisily sample $6$ times from the lemniscate sampler above, compute the persistence landscape for each point cloud, and then plot the persistence landscapes.
...
The purpose of the two sample Z-test is to test for the difference of means between two samples from the two distributions. The Z-test is a parametric test that requires the two samples to be normally distributed (a sample of persistence landscapes is a Gaussian random variable) and performs well when you have large sample sizes and known population variances. If these assumptions are not met, then the test is inappropriate. For more information, please reference section 3 of Bubenik (2015).

To conduct this test on persistent landscapes, our test statistic is the integral of the mean landscape. What interval this integration is taken over is dependent on the `support` provided. Users can enter intervals for the `support` parameter to focus on regions where important topological features lie, this also would increase efficiency as there would be less calculations to be made. Then, the Z-score is computed from which we get our p-value. We can adjust $\alpha$ by changing the `conf.level`. Here, we will use the default $0.95$ confidence level.
...
plz <- pl_z_test(pl_list_1,pl_list_2)
pval1 <- round(plz$p.value, 3)
```

Since the p-value is $`r pval1`$ at $\alpha=0.05$, we fail to reject the null hypothesis. This result suggests that, according to the Z-test, the samples appear to come from the same distribution. This highlights the Z-test's conservative nature when testing for differences between two samples of persistence landscapes.
...
```{r}
(perm <- pl_perm_test(pl_list_1,pl_list_2))
```
