...
To begin working with the persistence landscape framework, {plt} methods demands that our data is in a particular format. We import `as_persistence()` from [{phutil}](https://github.com/tdaverse/phutil) to coerce inputs into the `"persistence"` class, if your data is in one of the recognized formats/classes (see [persistence class vignette](https://github.com/tdaverse/phutil/blob/main/vignettes/persistence-class.qmd)).
