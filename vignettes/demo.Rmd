...
The plots below show the alpha complex at six different levels of the filtration parameter $\alpha$, which controls the radius of the balls.

```{r,echo = FALSE,fig.align = 'center'}
alpha_values <- c(0,0.05,0.1,0.175,0.35,0.4)

for (alpha in alpha_values) {
   alpha_shape <- ashape(X, alpha = alpha)
   plot(alpha_shape, col = c(1, 4), lty = c(1, 1), main = paste("Alpha Complex for alpha =", alpha)) 
   points(X, col = "red", pch = 16)
}
```

As $\alpha$ increases throughout these six plots, we observe that the number of connected components and loops changes. In addition, we also observe that some topological features, such as the two lobes of the figure-8, remain or persist throughout many $\alpha$'s.
